	...
	<c:forEach var="list" items="${list}">
      <tr onMouseOver="style.backgroundColor='#D1EEEE'" onMouseOut="style.backgroundColor=''">
        ...
        <font face="돋움" size="2" color="#000000">${listcount }</font></td>
				...
				<a href="pds_view.do?idx=${list.idx }">${list.subject}</a></font></td>
        <td align="center" height="20"><font face="돋움" size="2">${list.filename }</td>
				<td align="left" height="20"><font face="돋움" size="2"><a href="pds_list.do">
				${list.name }</a></font></td>
      ...
      <tr>
	   ...
	   <tr>
         ...
         <font face="돋움" size="2" color="#000000">${pageTag }</td>
		</tr>
   <tr>
	...
	<form name="pds01" method="post" action="pds_list.do">
     <table border="0" cellspacing="0" width="100%">
      ...
      <font color="#004080" size="4" face="Courier New"><strong>Search&nbsp;</strong></font>
        <select name="search" size="1" style="font-family: 돋움체">
		   <option value="subject" <c:if test="${search eq'subject'}"> select</c:if>> 글제목 </option>
		   <option value="name" <c:if test="${search eq'name'}"> select</c:if> > 글쓴이 </option>
		   <option value="contents"<c:if test="${search eq'contents'}">select</c:if> >글내용</option>
		</select>
		&nbsp;&nbsp;<input type="text" name="key" size="20" value ="${key }">
		&nbsp;&nbsp;<a href="javascript:sendding()"><input type="image" src="Pds/img/search2.gif" align="middle"></a>
	   </td>
	   </tr>
    </table>
    </form>
