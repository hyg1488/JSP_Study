...
function send(){
	alert("페이지를 이동합니다.");
	mm.action = "id_check.jsp";
	mm.submit();
}

	function id_close() {
		opener.mem.userid.value = '<%= id%>';
		self.close();
	}
	<%
		int cnt = 1;
		try{
		if(request.getParameter("h").equals("1")) cnt = 0;
...
<TR BGCOLOR=#948DCF>
  <TD>
    <TABLE CELLPADDING=4 CELLSPACING=1 BORDER=0 WIDTH=330>
        ...
        <% if(cnt == 0){ %>
        <td><input type ="hidden" name ="h" value ="0"></td>
    	...
    	<% } %>
           ...
           <INPUT NAME=userid type=text size=16 maxlength=16>
      	   <input type=image src="./img/u_bt08.gif" border=0 vspace=0 onclick ="send()">
		</form>
        </TD>
      </TR>
    </TABLE>
...
<TABLE CELLPADDING=0 CELLSPACING=0 BORDER=0 WIDTH=330>
  ...
  <tr>
    <td colspan=3 align=center>
      <a href="javascript:id_close()"><img src="./img/u_bt13.gif" border=0 vspace=5></a>
    </td>
  </tr>
</table>
</BODY>
