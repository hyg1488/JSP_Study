package exUsersServletJSTL.model;

import java.sql.Connection;
import java.sql.PreparedStatement;
import java.sql.ResultSet;

import exUsersServletJSTL.util.DBManager;
...
public class UserDAO {
	...
	public int userInsert(UserVO vo) {
		...
		int row = 0;
		String query = "insert into tbl_users(name, userid, passwd, tel, email, first_time) "
				+ "values(?, ?,?,?,?,SYSDATE)";
		try {
			// conn = getConnection();
	...
	public int userCheck(String userid) {
		...
		int row = 0;
		String query = "select userid from tbl_users where userid=?";
...
//					pstmt.getConnection().prepareStatement(query);
	...
	public int userModify(String passwd, String tel, String userid) {
		...
		int row = 0;
		String query = "update tbl_users set passwd=?,tel=? where userid=?";
		try {
			// conn = getConnection();
			conn = DBManager.getConnection();
			pstmt = conn.prepareStatement(query);
